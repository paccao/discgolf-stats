...
async function main() {
  const courseDalum = await prisma.course.upsert({
    ...
    create: {
      ...
      description:
        'Dalum DiscGolfPark is suitable for all players and experience levels. A shorter forest course in beautiful surroundings.',
      layout: 'Gold',
      location: 'Dalum',
      baskets: {
        createMany: {
  ...
  const courseAle = await prisma.course.upsert({
    ...
    create: {
      ...
      description:
        'Stunningly beautiful course, great shot variety required (mix of par 3s, 4s, 5s). Tops of elevation change. Mix of open and tight holes. Clean fairways, well maintained, great flow.',
      location: 'Nol',
      layout: 'White web',
      baskets: {
        createMany: {
